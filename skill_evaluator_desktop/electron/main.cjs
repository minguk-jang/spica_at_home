...
const repoRoot = path.resolve(appRoot, "..", "skill_evaluator");
const defaultDbPath = path.join(repoRoot, "outputs", "webmcp_plugin_cold_iter_check", "workflows.sqlite");
const defaultOutputDir = path.join(repoRoot, "outputs", "desktop_runs");
const defaultPythonPath = path.join(repoRoot, "reference", "webwright", ".venv", "bin", "python");
