...
} from "../widgets/workflow-detail/model/script-preview";
import {
  LatestRunResult,
  ProposalList,
  ResultSummaryPanel,
  RunsView,
  UpdatesView
} from "../widgets/workflow-detail/ui";
import { RunEvents } from "../widgets/run-queue";
import {
  UPDATE_MODE_OPTIONS,
  discoveryProviderForUpdateMode,
  type UpdateMode
} from "../features/update-workflow/model/updateModeOptions";
import {
  activeJobControlLabel,
  activeJobStatusText,
  activeJobTitle,
  type ActiveJob
} from "../features/active-job/model/activeJob";
import {
  summarizeEvolutionJobStatus,
  summarizeEvolutionOutput
} from "../features/evolve-workflow/model/evolutionSummary";
import {
  browserModeLabel,
  evolutionStatusLabel
} from "../features/evolve-workflow/model/evolutionDisplay";
import {
  defaultJsToolArgumentsJson,
  defaultRequiredOutputKeys
} from "../features/js-tool/model/jsToolDefaults";
import {
  buildVisibleRunControlFields,
  type VisibleRunControlField
} from "../features/run-workflow/model/runControlFields";
import { defaultRunValue } from "../features/run-workflow/model/runDefaults";
import { buildRunArgumentPayload } from "../features/run-workflow/model/runPayload";
import {
  argumentExampleMeta,
  argumentDisplayRows,
  ...
  type CreateWorkflowStepGuideItem
} from "../features/create-workflow/model/workflowDashboard";
import { normalizeWorkflowDetail } from "../entities/workflow/model/detailDefaults";
import {
  coreLogicStages,
  desktopTabGuides,
  landingMetricItems,
...
} from "../pages/home/model/landingPage";
import {
  Badge,
  CheckboxField,
  IconButton,
...
import { numberFromUnknown, stringFromUnknown } from "../shared/lib/coerce";
import { parseJsonObject, parseRequiredOutputKeys } from "../shared/lib/json";
...
const workflowStepGuideTypeOptions = [
  "goto",
  "click",
  "click_text",
  "fill",
...
export function App(): React.ReactElement {
  ...
  const runArgumentValuesRef = useRef({
    companyName,
    ticker,
    newsLimit,
    extraArguments
  ...
  const [createStepGuide, setCreateStepGuide] = useState<CreateWorkflowStepGuideItem[]>([]);
  ...
  const [stepGuideSuggestionEvent, setStepGuideSuggestionEvent] = useState<RunEvent | null>(null);
  const [suggestingStepGuide, setSuggestingStepGuide] = useState(false);
  ...
  function selectedRunArgumentPayload(): Pick<RunPayload, "companyName" | "ticker" | "newsLimit" | "extraArguments"> {
    return buildRunArgumentPayload(detail?.arguments ?? [], {
      ...runArgumentValuesRef.current
    });
  }
  ...
  async function suggestStepGuideDraft(): Promise<void> {
    if (!window.webmcp || !canCreateWorkflow({
      startUrl: createStartUrl,
      task: createTask,
      finalState: createFinalState
    ...
    setSuggestingStepGuide(true);
    setStatus("Suggesting step guide draft");
    try {
      const payload: StepGuideSuggestionPayload = {
        dbPath,
        repoRoot,
        pythonPath,
        startUrl: createStartUrl.trim(),
        task: createTask.trim(),
        finalState: createFinalState.trim(),
        synthesizerModel: updateModel
      };
      const event = await window.webmcp.suggestStepGuide(payload);
      const draft = stepGuideItemsFromSuggestionOutput(event.output);
      setStepGuideSuggestionEvent(event);
      if (draft.length > 0) {
        setCreateStepGuide(draft);
        setStatus(`Suggested ${draft.length} workflow steps`);
      } else {
        setStatus("Step guide suggestion returned no editable steps");
      }
    } catch (error: unknown) {
      setStatus(webmcpErrorMessage(error));
    } finally {
      setSuggestingStepGuide(false);
    }
  }
  ...
  return (
    <main className="appShell">
      ...
      <CreateWorkflowSheet
        ...
        stepGuide={createStepGuide}
        setStepGuide={setCreateStepGuide}
        headed={createHeaded}
        ...
        latestSuggestionEvent={stepGuideSuggestionEvent}
        suggesting={suggestingStepGuide}
        onOpenTarget={(targetPath) => void window.webmcp?.openPath(targetPath)}
        onSuggest={() => void suggestStepGuideDraft()}
...
function CreateWorkflowSheet(props: {
  ...
  setStepGuide: (value: CreateWorkflowStepGuideItem[]) => void;
  headed: boolean;
  ...
  latestEvent: RunEvent | null;
  latestSuggestionEvent: RunEvent | null;
  suggesting: boolean;
  onOpenTarget: (path: string) => void;
  onSuggest: () => void;
  onCreate: () => void;
  ...
  function addStepGuideItem(): void {
    props.setStepGuide([
      ...props.stepGuide,
      {
        name: `step_${props.stepGuide.length + 1}`,
        description: "",
        stepType: props.stepGuide.length === 0 ? "goto" : "click"
      }
    ]);
  }
  function updateStepGuideItem(index: number, patch: Partial<CreateWorkflowStepGuideItem>): void {
    props.setStepGuide(
      props.stepGuide.map((item, itemIndex) => (itemIndex === index ? { ...item, ...patch } : item))
    );
  }
  function removeStepGuideItem(index: number): void {
    props.setStepGuide(props.stepGuide.filter((_, itemIndex) => itemIndex !== index));
  }
  function duplicateStepGuideItem(index: number): void {
    const item = props.stepGuide[index];
    if (!item) {
      return;
    }
    const duplicate = {
      ...item,
      name: `${item.name || `step_${index + 1}`}_copy`
    };
    props.setStepGuide([
      ...props.stepGuide.slice(0, index + 1),
      duplicate,
      ...props.stepGuide.slice(index + 1)
    ]);
  }
  function moveStepGuideItem(fromIndex: number, toIndex: number): void {
    if (fromIndex === toIndex || fromIndex < 0 || toIndex < 0 || fromIndex >= props.stepGuide.length || toIndex >= props.stepGuide.length) {
      return;
    }
    const next = [...props.stepGuide];
    const [moved] = next.splice(fromIndex, 1);
    next.splice(toIndex, 0, moved);
    props.setStepGuide(next);
  }
  return (
    <div className="modalBackdrop" role="presentation">
      <section className="modalPanel createPanel" role="dialog" aria-modal="true" aria-labelledby="create-workflow-title">
        ...
        <div className="modalBody createBody">
          ...
          <section className="stepGuideEditor" aria-label="Step guide">
            <div className="stepGuideHeader">
              <div>
                <p className="eyebrow">Guide</p>
                <h3>Step guide</h3>
              </div>
              <div className="stepGuideActions">
                <button
                  className="secondaryButton iconTextButton"
                  type="button"
                  onClick={props.onSuggest}
                  disabled={!ready || props.suggesting}
                >
                  <Sparkles size={16} aria-hidden="true" />
                  <span>{props.suggesting ? "Suggesting" : "Suggest Draft"}</span>
                </button>
                <button className="secondaryButton iconTextButton" type="button" onClick={addStepGuideItem}>
                  <Plus size={16} aria-hidden="true" />
                  <span>Add Step</span>
                </button>
              </div>
            </div>
            {props.latestSuggestionEvent ? (
              <div className="stepGuideSuggestionMeta">
                <Sparkles size={15} aria-hidden="true" />
                <span>{stepGuideSuggestionSummary(props.latestSuggestionEvent)}</span>
              </div>
            ) : null}
            {props.stepGuide.length > 0 ? (
              <div className="stepGuideList">
                {props.stepGuide.map((item, index) => (
                  <div
                    className={`stepGuideRow${draggingStepIndex === index ? " dragging" : ""}`}
                    draggable
                    key={`step-guide-${index}`}
                    onDragStart={() => setDraggingStepIndex(index)}
                    onDragEnd={() => setDraggingStepIndex(null)}
                    onDragOver={(event) => event.preventDefault()}
                    onDrop={() => {
                      if (draggingStepIndex !== null) {
                        moveStepGuideItem(draggingStepIndex, index);
                      }
                      setDraggingStepIndex(null);
                    }}
                  >
                    <div className="stepGuideRowHeader">
                      <div className="stepGuideRowLabel">
                        <GripVertical size={16} aria-hidden="true" />
                        <span>Step {index + 1}</span>
                      </div>
                      <div className="stepGuideRowTools">
                        <IconButton
                          label={`Move step ${index + 1} up`}
                          title={`Move step ${index + 1} up`}
                          onClick={() => moveStepGuideItem(index, index - 1)}
                          disabled={index === 0}
                          variant="plain"
                        >
                        ...
                        <IconButton
                          ...
                          onClick={() => moveStepGuideItem(index, index + 1)}
                          disabled={index === props.stepGuide.length - 1}
                          variant="plain"
                        >
                        ...
                        <IconButton
                          ...
                          onClick={() => duplicateStepGuideItem(index)}
                          variant="plain"
                        >
                        ...
                        <IconButton
                          ...
                          onClick={() => removeStepGuideItem(index)}
                          variant="plain"
                        >
                    ...
                    <div className="stepGuideFields">
                      <TextField
                        label="Name"
                        value={item.name}
                        onChange={(value) => updateStepGuideItem(index, { name: value })}
                      />
                      <SelectField
                        label="Type"
                        value={item.stepType}
                        options={workflowStepGuideTypeOptions}
                        onChange={(value) => updateStepGuideItem(index, { stepType: value })}
                      />
                      <label className="textAreaField stepGuideDescription">
                        <span>Description</span>
                        <textarea
                          value={item.description}
                          rows={2}
                          onChange={(event) => updateStepGuideItem(index, { description: event.target.value })}
                        />
                      </label>
                    </div>
                  </div>
              ...
              <div className="emptyGuideState">
                <ListChecks size={18} aria-hidden="true" />
                <span>Add rough steps when the task needs a human scaffold.</span>
              </div>
            )}
          </section>
...
function JsToolStudio(props: {
  ...
  const missingOutputKeys = toStringList(lastOutput.missing_output);
  const latestEventKind = props.latestEvent?.type ?? "";
  const hasRunEvidence = latestEventKind === "js-tool-run-finished" || latestEventKind === "js-tool-eval-finished";
  const hasEvalEvidence = latestEventKind === "js-tool-eval-finished";
  ...
  const toolDirLabel = canUseExportedTool ? props.toolDir.trim() : "아직 변환된 JS 도구가 없습니다.";
  const resultInsight = jsToolResultInsight(props.latestEvent, lastOutput, missingOutputKeys);
  return (
    <section className="tabPanel jsToolStudio">
      <div className="jsToolHero">
        <div>
          ...
          <h3>Workflow를 JS 도구로 변환하고 실제 입력으로 검증합니다.</h3>
          <p>
            선택한 버전을 Node.js에서 호출 가능한 도구로 바꾸고, 테스트 입력 JSON으로 실행 결과와 출력 계약을 확인합니다.
          </p>
        </div>
        <span className="metaRow">
          ...
          <Badge>{requiredOutputKeys.length}개 출력 검증</Badge>
        </span>
      </div>

      <div className="jsToolFlow" aria-label="JavaScript tool workflow">
        <JsToolStageCard
          index={1}
          icon={<FileText size={16} aria-hidden="true" />}
          title="JS 도구로 변환"
          status={canUseExportedTool ? "준비됨" : "대기"}
          tone={canUseExportedTool ? "success" : "idle"}
        >
          Workflow 버전을 JS runtime이 읽을 수 있는 도구 폴더로 만듭니다.
        </JsToolStageCard>
        <JsToolStageCard
          index={2}
          icon={<Play size={16} aria-hidden="true" />}
          title="Node.js 실행 검증"
          status={hasRunEvidence ? "실행됨" : canUseExportedTool ? "준비됨" : "대기"}
          tone={hasRunEvidence ? "success" : canUseExportedTool ? "ready" : "idle"}
        >
          page_text가 필요한 단계까지 포함해 run contract와 출력 payload를 확인합니다.
        </JsToolStageCard>
        <JsToolStageCard
          index={3}
          icon={<ShieldCheck size={16} aria-hidden="true" />}
          title="Eval로 개선 포인트 확인"
          status={hasEvalEvidence ? (evalPassed ? "통과" : "확인 필요") : "대기"}
          tone={hasEvalEvidence ? (evalPassed ? "success" : "warning") : "idle"}
        >
          필수 출력 키 누락과 실패 이유를 보고 workflow 수정 필요 여부를 판단합니다.
        </JsToolStageCard>
      </div>

      <div className="jsToolWorkGrid">
        <section className="jsToolPanel">
          <div className="jsToolPanelHeader">
            <SectionTitle icon={<FileText size={17} />} title="1. 변환" />
            <Badge>{selectedVersion ? `v${selectedVersion.version}` : "version -"}</Badge>
          </div>
          <p className="jsToolHelp">
            DB에 저장된 현재 workflow 버전을 JS 도구로 변환합니다. 변환이 끝나면 아래 실행 검증 버튼이 활성화됩니다.
          </p>
          <div className="jsToolActionRow">
            <button
              className="primaryButton jsToolActionButton"
              type="button"
              aria-label="JS 도구로 변환"
              title="JS 도구로 변환"
          ...
          <div className="jsToolPath">
            <span>현재 도구</span>
            <strong>{toolDirLabel}</strong>
          </div>
          <details className="jsToolAdvanced">
            <summary>고급 설정</summary>
            <TextField label="저장 위치" value={props.outputDir} onChange={props.setOutputDir} />
            <TextField label="도구 폴더" value={props.toolDir} onChange={props.setToolDir} />
          </details>
          <details className="jsToolDetails">
            <summary>변환 산출물</summary>
        ...
        <section className="jsToolPanel">
          <div className="jsToolPanelHeader">
            <SectionTitle icon={<Play size={17} />} title="2. 실행 검증" />
            <Badge>Node.js 입력</Badge>
          </div>
          <p className="jsToolHelp">
            아래 JSON을 동일한 arguments로 넘겨 변환된 JS 도구가 실행 가능한지 확인합니다. 브라우저 DOM이 필요한 workflow는 page_text 입력으로 계약을 검증합니다.
          </p>
          <label className="textAreaField">
            <span>테스트 입력 JSON</span>
            <textarea
        ...
        <section className="jsToolPanel jsToolPanelWide">
          <div className="jsToolPanelHeader">
            <SectionTitle icon={<ShieldCheck size={17} />} title="3. Eval 분석" />
            <Badge>{requiredOutputKeys.length}개 키</Badge>
          </div>
          <div className="jsToolEvalLayout">
            <div className="jsToolEvalControls">
              <p className="jsToolHelp">
                Eval은 JS 도구 실행 결과가 필수 출력 계약을 만족하는지 확인하고, 빠진 값이 있으면 개선 포인트로 보여줍니다.
              </p>
              <label className="textAreaField">
                <span>검증할 출력 키</span>
                <textarea
              ...
              <div className="jsToolActionRow">
                <button
                  ...
                  type="button"
                  aria-label="JS 도구 Eval 분석"
                  title="JS 도구 Eval 분석"
                  disabled={props.running || !canUseExportedTool}
                  onClick={props.onEval}
                >
                  <ShieldCheck size={16} aria-hidden="true" />
                  <span>Eval 분석 실행</span>
                </button>
              </div>
              <div className="jsToolFacts">
                <SummaryItem label="지원 단계" value="deterministic step" />
                <SummaryItem label="런타임" value="Node.js CommonJS" />
                <SummaryItem label="출력 계약" value={requiredOutputKeys.length === 0 ? "-" : requiredOutputKeys.join(", ")} />
              </div>
            </div>

            <div className="jsToolResult">
              <SectionTitle icon={<Activity size={17} />} title="최근 결과" />
              {props.latestEvent ? (
                <>
                  <div className="resultPanelHeader">
                    <div>
                      <h4>{jsToolResultTitle(props.latestEvent)}</h4>
                      <span className="metaRow">
                  ...
                  <div className={`jsToolResultSummary ${evalPassed ? "passed" : missingOutputKeys.length > 0 ? "warning" : ""}`}>
                    <strong>{resultInsight.title}</strong>
                    <p>{resultInsight.description}</p>
                    {missingOutputKeys.length > 0 ? (
                      <div className="jsToolOutputList" aria-label="Missing output keys">
                        {missingOutputKeys.map((key) => <Badge key={key}>{key}</Badge>)}
                      </div>
                    ) : null}
                  </div>
                  {props.latestEvent.stderr ? <pre className="stderr">{props.latestEvent.stderr}</pre> : null}
                  <details className="jsToolDetails">
                    <summary>원본 결과 JSON</summary>
...
type JsToolStageTone = "idle" | "ready" | "success" | "warning";

function JsToolStageCard({
  index,
  icon,
  title,
  status,
  ...
  title: string;
  status: string;
  tone: JsToolStageTone;
  children: React.ReactNode;
}): React.ReactElement {
  return (
    <article className={`jsToolStageCard ${tone}`}>
      <div className="jsToolStageTop">
        <span className="jsToolStageIcon">{icon}</span>
        <span className="jsToolStageIndex">{String(index).padStart(2, "0")}</span>
      </div>
      <div>
        <h4>{title}</h4>
        <p>{children}</p>
      </div>
      <span className={`jsToolStageStatus ${tone}`}>{status}</span>
    </article>
  );
}

function jsToolResultTitle(event: RunEvent): string {
  if (event.type === "js-tool-export-finished") {
    return "변환 완료";
  }
  if (event.type === "js-tool-run-finished") {
    return "실행 검증 완료";
  }
  if (event.type === "js-tool-eval-finished") {
    return "Eval 분석 완료";
  }
  return event.type.replaceAll("-", " ");
}

function jsToolResultInsight(
  event: RunEvent | null,
  ...
  if (!event) {
    return {
      title: "결과 대기 중",
      description: "JS 도구로 변환하거나 실행 검증을 마치면 결과가 여기에 표시됩니다."
    };
  }
  if (event.type === "js-tool-export-finished") {
    return {
      title: "JS 도구 변환이 끝났습니다.",
      description: "테스트 입력 JSON으로 실행 검증을 진행할 수 있습니다."
    };
  }
  if (event.type === "js-tool-run-finished") {
    const status = firstStringFromKeys(output, "status") ?? event.status ?? "finished";
    const message = firstStringFromKeys(output, "message", "error");
    if (status === "failed" || event.status === "failed") {
      return {
        title: "JS 도구 실행이 실패했습니다.",
        description: message ? cleanJsToolMessage(message) : "테스트 입력 JSON과 원본 결과 JSON을 확인해 실패한 단계의 입력을 보강하세요."
      };
    }
    return {
      title: `실행 상태: ${status}`,
      description: "출력 계약까지 확인하려면 Eval 분석 실행을 누르세요."
    };
  }
  if (event.type === "js-tool-eval-finished") {
    const message = firstStringFromKeys(output, "message", "error");
    if (output.status === "failed" || event.status === "failed") {
      return {
        title: "Eval 분석이 실패했습니다.",
        description: message ? cleanJsToolMessage(message) : "원본 결과 JSON을 확인해 실행 실패나 계약 불일치 원인을 좁히세요."
      };
    }
    if (output.passed === true) {
      return {
        title: "출력 계약을 통과했습니다.",
        description: "동일한 입력으로 변환된 JS 도구가 필수 출력 값을 모두 만들었습니다."
      };
    }
    if (missingOutputKeys.length > 0) {
      return {
        title: "누락된 출력 키가 있습니다.",
        description: "아래 키를 만들도록 workflow output schema, handler, report 단계를 수정해야 합니다."
      };
    }
    return {
      title: "Eval 결과 확인이 필요합니다.",
      description: "원본 결과 JSON과 stderr를 확인해 실행 실패나 계약 불일치 원인을 좁히세요."
    };
  }
  return {
    title: event.status ?? "작업 결과",
    description: "원본 결과 JSON에서 세부 상태를 확인할 수 있습니다."
  };
}

function cleanJsToolMessage(message: string): string {
  return message.replace(/^javascript tool failed:\s*/i, "");
}
...
function stepGuideItemsFromSuggestionOutput(output: unknown): CreateWorkflowStepGuideItem[] {
  const record = asRecord(output);
  const source = Array.isArray(record.step_guide) ? record.step_guide : [];
  return source
    .map((item): CreateWorkflowStepGuideItem | null => {
      const step = asRecord(item);
      const name = stringFromUnknown(step.name).trim();
      const description = stringFromUnknown(step.description).trim();
      const stepType = stringFromUnknown(step.step_type || step.stepType).trim();
      if (!name && !description) {
        return null;
      }
      return {
    ...
    .filter((item): item is CreateWorkflowStepGuideItem => item !== null);
}

function stepGuideSuggestionSummary(event: RunEvent): string {
  const output = asRecord(event.output);
  const provider = stringFromUnknown(output.provider || event.status || "finished").trim();
  const stepCount = Array.isArray(output.step_guide) ? output.step_guide.length : 0;
  if (event.exitCode !== undefined && event.exitCode !== 0) {
    return `Suggestion failed: ${event.stderr || event.status || "unknown error"}`;
  }
  return `${stepCount} draft steps from ${provider}`;
}
